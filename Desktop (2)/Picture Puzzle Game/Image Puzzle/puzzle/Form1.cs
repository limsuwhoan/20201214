...
namespace WindowsFormsApplication1
{
    ...
    { 
        ...
        private void btnShuffle_Click(object sender, EventArgs e)
        {
            ...
            {
                YesOrNo = MessageBox.Show("다시 시작 하겠습니까??", "Rabbit Puzzle", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            }
        ...
        {
            DialogResult YesOrNO = MessageBox.Show("게임을 종료하시곘습니까??", "Rabbit Puzzle", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            if (inNullSliceIndex != inPictureBoxIndex)
            {
                ...
                if (FourBrothers.Contains(inNullSliceIndex))
                {
                    ...
                    if (CheckWin())
                    {
                        ...
                        MessageBox.Show("축하합니다!!\nYour Rabbit Is Happy!!\n시간 : " + timer.Elapsed .ToString().Remove(8)+"초가 걸렸고"+ "\n 이동횟수는 : " + inmoves, "Rabbit Puzzle");
                        inmoves = 0;
                        lblMovesMade.Text = "Moves Made : 0";
                        lblTimeElapsed.Text = "00:00:00";
                        timer.Reset();
        ...
        {
 MessageBox.Show("●게임 설명 : 왼쪽의 퍼즐을 옮겨 오른쪽 이미지와 같은 형태로 " +
                "만드는 이미지 퍼즐 맞추기 게임 " +
                 "\n" + "게임시작은 퍼즐 클릭시 바로 시작됩니다.  " +
                "\n" +
                "\n" +
                "\n" +"ex)"+
                "\n" + "[shuffle] 클릭시 시작전엔 퍼즐섞기 게임 시작후엔 다시하기 기능" +
                "\n" + "[pause] 클릭시 일시정지 기능" +
                "\n" + "[quit] 클릭시 게임 종료 기능"
                );
        }
